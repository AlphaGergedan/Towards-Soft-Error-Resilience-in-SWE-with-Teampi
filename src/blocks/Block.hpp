...
#ifndef SWE_BLOCK_HPP
#define SWE_BLOCK_HPP
...
public:
    ...
    int maxDivisor = 1024;   // smallest timestep possible maxTimestepLocal/maxDivisor
    ...
    GhostlayerState receivedGhostlayer[4];  // determines if border received a valid timestep, thus is bigger or same
                                            // then localtimestep.
...
void SWE_Block<T, Buffer>::printLtsStats() {
    std::cout << myRank << "|" << timestepCounter << "|" << iteration << "| L(" << neighbourRankId[BND_LEFT] << ","
              << receivedGhostlayer[BND_LEFT] << "): " << std::setprecision(6) << std::fixed << borderTimestep[BND_LEFT]
              << " R(" << neighbourRankId[BND_RIGHT] << "," << receivedGhostlayer[BND_RIGHT]
              << "): " << borderTimestep[BND_RIGHT] << " B(" << neighbourRankId[BND_BOTTOM] << ","
              << receivedGhostlayer[BND_BOTTOM] << "): " << borderTimestep[BND_BOTTOM] << " T("
              << neighbourRankId[BND_TOP] << "," << receivedGhostlayer[BND_TOP] << "): " << borderTimestep[BND_TOP]
              << " | " << getTotalLocalTimestep() << " " << std::endl;
}
...
void SWE_Block<T, Buffer>::checkAllGhostlayers() {
    if (localTimestepping) {
        // if neighbour stepped further than the local block, interpolation is possible, if not, block shall wait for
        // fitting timestep
        for (int border = BND_LEFT; border <= BND_TOP; border++) {
            if ((boundaryType[border] == CONNECT) || boundaryType[border] == CONNECT_WITHIN_RANK) {
                if (std::fabs(borderTimestep[border] - getTotalLocalTimestep()) <=
                    std::numeric_limits<double>::epsilon()) {
                    // This case the neighbor progressed as much as local block did and thus the received Ghostlayer is
                    // valid and we can expect
                    ...
                    // in this case we need to wait for the next iteration to receive a valid timestep, block will not
                    // compute until received a valid timestep.
        ...
        if (std::fabs(duration - getTotalLocalTimestep()) <= std::numeric_limits<double>::epsilon() ||
            getTotalLocalTimestep() >= duration) {
...
float SWE_Block<T, Buffer>::getTotalLocalTimestep() {
    ...
    return (float)(maxTimestepLocal * timestepCounter) +
           (stepSize > 0 ? ((float)(maxTimestepLocal * stepSizeCounter) / (float)stepSize) : 0);
}
